...
class CompanyDaoTestSuite {
    ...
    @Test
    void testEmployeeLastNameAndCompanyThreeLeter(){
        Employee johnSmith = new Employee("John", "Smith");
        Employee stephanieClarckson = new Employee("Stephanie", "Clarckson");
        Employee lindaKovalsky = new Employee("Linda", "Kovalsky");
        ...
        companyDao.save(greyMatter);
        int greyMatterId = greyMatter.getId();
        List<Employee> retriveLastName = employeeDao.retrieveLastName("Smith");
        List<Company> retriveFirstThreeLeterOfCompany = companyDao.retriveFirstThreeLeters("Sof");

        //Then
        assertEquals("Smith", retriveLastName.get(0).getLastName());
        assertEquals("Software Machine", retriveFirstThreeLeterOfCompany.get(0).getName());
